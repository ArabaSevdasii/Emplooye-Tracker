"""
Main screen: left/right split panel.

Left: default / new employee / employee detail (daily) / edit / past payments.
Right: employee list, weekly total, ADD NEW EMPLOYEE, PAYMENT HISTORY, RESET ENTIRE WEEK, REMOVE.
"""
import datetime
...
def _format_history_header(ts_text: str) -> str:
    ...
    for fmt in ("%Y-%m-%d %H:%M:%S", "%Y-%m-%d"):
        try:
            ...
            continue
    if not dt:
        return f"====== {ts_text} Transactions ======"
    day = f"{dt.day:02d}"
    month = MONTH_EN.get(dt.month, str(dt.month))
    year = dt.year
    if " " in ts_text:
        return f"====== {month} {day}, {year} - Time: {dt.hour:02d}:{dt.minute:02d} Transactions ======"
    return f"====== {month} {day}, {year} Transactions ======"
...
def _missing_summary_for_days(day_values):
    ...
    for i, v in enumerate(day_values):
        h = int(v) if v else 0
        if h > 0:
            parts.append(f"{labels[i]}: -{h} hours")
...
class DashboardView(ctk.CTkFrame):
    ...
    def _show_default_view(self):
        ...
        ctk.CTkLabel(
            self.left_panel,
            text="Select a person from the right to perform an action.",
    ...
    def _on_global_reset(self):
        if not messagebox.askyesno(
            "Confirm",
            "All employee hour deductions will be reset. Are you sure?",
            parent=self.winfo_toplevel(),
        ):
            return

        # Check: actual_paid > net_paid?
        ...
        for row in employees:
            ...
            if float(actual_paid_val) > net_paid + 0.009:
                problem_list.append(
                    f"  • {name}: Manually Paid {actual_paid_val:,.2f} ₺ > Net {net_paid:,.2f} ₺"
                )
        ...
        if problem_list:
            ...
            messagebox.showerror(
                ...
                f"The manually paid amount for the following employees is greater than their calculated net salary!\n"
                f"Please fix these employees first:\n\n{problem_str}\n\n"
                f"Reset was not performed.",
                parent=self.winfo_toplevel(),
            )
            return
    ...
    def _show_add_employee(self):
        ...
        ctk.CTkLabel(main, text="New Employee", font=self.font_title).pack(anchor="w", pady=(0, 20))
        ctk.CTkLabel(main, text="Full Name", font=self.font).pack(anchor="w", pady=(0, 4))
        entry_name = ctk.CTkEntry(main, height=48, font=self.font)
        entry_name.pack(fill="x", pady=(0, 16))
        ctk.CTkLabel(main, text="Phone", font=self.font).pack(anchor="w", pady=(0, 4))
        entry_phone = ctk.CTkEntry(main, height=48, font=self.font)
        entry_phone.pack(fill="x", pady=(0, 16))
        ctk.CTkLabel(main, text="Weekly Salary ($)", font=self.font).pack(anchor="w", pady=(0, 4))
        entry_salary = ctk.CTkEntry(main, height=48, font=self.font, placeholder_text="E.g. 2500")
        ...
        def save():
            ...
            if not name:
                ...
                return
            if not salary_str:
                err.configure(text="Weekly salary cannot be empty.")
                return
            try:
                ...
                err.configure(text="Enter a valid salary.")
                return
            err.configure(text="")
        ...
        ctk.CTkButton(
            btn_f, text="SAVE", font=self.font_big, height=60,
            fg_color="green", hover_color="darkgreen", command=save,
        ).pack(side="left", fill="x", expand=True, padx=(0, 8))
        ctk.CTkButton(
            btn_f, text="CANCEL", font=self.font_big, height=60,
    ...
    def _show_employee_detail(self, employee_id):
        ...
        self._clear_left()
        # ScrollableFrame so all days are reachable
        ...
        ctk.CTkLabel(main, text="Employee", font=self.font).pack(anchor="w", pady=(0, 4))
        ...
        ctk.CTkButton(
            top_row, text="EDIT EMPLOYEE", font=self.font_day_status, height=40,
            command=lambda: self._show_edit_employee(employee_id),
        ).grid(row=0, column=1, sticky="e", padx=(12, 0))

        ctk.CTkLabel(main, text="Weekly Salary (Base)", font=self.font).pack(anchor="w", pady=(0, 4))
        ...
        ctk.CTkLabel(
            actual_inner,
            text="Amount Paid Manually This Week (₺):",
            font=self.font,
            anchor="w",
        ...
        actual_entry = ctk.CTkEntry(
            ...
            height=48,
            font=self.font_big,
            placeholder_text="Cannot exceed net salary",
        )
        ...
        def _save_actual_paid():
            ...
            try:
                ...
                if val < 0:
                    raise ValueError()
                if val > net():
                    actual_err_lbl.configure(text=f"⚠ Cannot exceed net salary ({net():,.2f} ₺)!")
                    return
            except ValueError:
                actual_err_lbl.configure(text="Enter a valid amount.")
                return
            actual_err_lbl.configure(text="")
        ...
        def refresh_money():
            n = net()
            net_lbl.configure(text=f"NET SALARY TO PAY: {n:,.2f} ₺")
            deduction = max(0.0, weekly_salary - n)
            deduction_lbl.configure(text=f"Total Deduction This Week: {deduction:,.2f} ₺")
            try:
                raw = actual_entry.get().strip().replace(",", ".")
                if raw:
                    val = float(raw)
                    if val > n:
                        actual_err_lbl.configure(text=f"⚠ Cannot exceed net salary ({n:,.2f} ₺)!")
                    else:
                        actual_err_lbl.configure(text="")
            except Exception:
                pass
        ...
        ctk.CTkLabel(main, text="Daily absence (Mon–Fri):", font=self.font).pack(anchor="w", pady=(6, 4))
        status_lbls = []

        def _update_status(idx):
            h = float(day_vals[idx] or 0)
            if h > 0:
                status_lbls[idx].configure(text=f"{DAY_NAMES[idx]}: {int(h)} Hours Missing", text_color="red")
            else:
                status_lbls[idx].configure(text=f"{DAY_NAMES[idx]}: Present", text_color="green")
        ...
        for day_index, day_name in enumerate(DAY_NAMES):
            ...
            ctk.CTkButton(
                btn_row, text="Reset", font=self.font_day_status, height=38,
        ...
        def _toggle_paid():
            ...
            if current_paid == 1:
                if not messagebox.askyesno(
                    "Confirm",
                    f"The payment mark for {name} will be UNDONE.\nAre you sure?",
                    parent=self.winfo_toplevel(),
                ):
                    return
        ...
        if is_paid:
            pay_btn_text = "✅ PAID — Undo"
            pay_btn_color = "#5a7a5a"
            pay_btn_hover = "#4a6a4a"
        else:
            pay_btn_text = "MARK AS PAID"
    ...
    def _show_edit_employee(self, employee_id):
        ...
        ctk.CTkLabel(main, text="Edit Employee", font=self.font_title).pack(anchor="w", pady=(0, 20))
        ctk.CTkLabel(main, text="Full Name", font=self.font).pack(anchor="w", pady=(0, 4))
        entry_name = ctk.CTkEntry(main, height=48, font=self.font)
        entry_name.pack(fill="x", pady=(0, 16))
    ...
    def _on_clear_history(self):
        if not messagebox.askyesno(
            "Confirm",
            "All past payment records will be deleted. Are you sure?",
            parent=self.winfo_toplevel(),
        ):
            return
        clear_payment_history()
    ...
    def _show_payment_history(self):
        ...
        ctk.CTkLabel(main, text="Payment History", font=self.font_title).pack(anchor="w", pady=(0, 16))
        ctk.CTkButton(
            main,
            text="🗑️ Delete Past Data",
            font=self.font_big,
            height=56,
            fg_color="#8B0000",
            hover_color="#5C0000",
        ...
        for r in records:
            ...
            if payment_date != last_ts:
                if last_ts is not None:
                    ctk.CTkLabel(
                        scroll,
                        text=f"💰 Total Paid in This Batch: {batch_total:,.2f} ₺",
            ...
            if actual_paid_h is not None:
                ...
                if diff > 0.009:
                    actual_part = f" | Manually Paid: {actual_paid_h:,.2f} ₺ | ⚠ Missing Payment: -{diff:,.2f} ₺"
                else:
                    actual_part = f" | Manually Paid: {actual_paid_h:,.2f} ₺ ✓"
            text = f"{emp_name} | Calculated Net: {net_paid:,.2f} ₺{actual_part} | Details: {missing_summary or 'None'}"
    ...
    def _on_remove_employee(self, emp_id: int):
        if not messagebox.askyesno(
            "Confirm",
            "Are you sure you want to delete this employee? (Past payment records are not deleted)",
            parent=self.winfo_toplevel(),
        ):
            return
    ...
    def _refresh_employee_list(self):
        ...
        for row in employees:
            ...
            eksik_text = "None" if total_missing == 0 else f"{total_days} Days, {hours_rem} Hours"
            ...
            detail_lbl = ctk.CTkLabel(
                left_block,
                text=f"Base: {weekly_salary:,.2f} ₺  |  Net: {net_salary:,.2f} ₺  |  Missing: {eksik_text}",
            ...
            if actual_paid_val is not None:
                ...
                if diff > 0.009:
                    diff_color = "orange"
                    diff_text = f"Manually Paid: {actual_paid_val:,.2f} ₺  |  Missing Payment: -{diff:,.2f} ₺"
                else:
                    diff_color = "green"
                    diff_text = f"Manually Paid: {actual_paid_val:,.2f} ₺  ✓ Fully Paid"
                ctk.CTkLabel(
        ...
        self.total_weekly_label.configure(text=f"💰 TOTAL TO PAY THIS WEEK: {grand_total:,.2f} ₺")